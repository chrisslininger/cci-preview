...
export default function ControlPanel() {
  ...
  return <>
    ...
    <div className="ptbl"><table><thead><tr><th>Person</th><th>Roles</th><th>Last sign-in</th><th>Last action</th><th>Silent</th></tr></thead><tbody>
      {people.map((p) => { const d = daysSince(lastSeen(p)); return <tr key={p.id} className={d == null || d >= 30 ? 'quiet' : ''}><td><a className="plink" href="#leads">{p.name}</a>{!p.has_login && <><br /><small className="muted">no login yet</small></>}</td><td><small>{rolesText(p)}</small></td><td>{p.last_sign_in_at ? fmtTs(p.last_sign_in_at) : <i>never</i>}</td><td>{p.last_activity_at ? <>{actionText({ kind: p.last_kind as Activity['kind'], tab: p.last_tab, detail: p.last_detail })}<small className="muted"> · {fmtTs(p.last_activity_at)}</small></> : <i>no activity recorded yet</i>}</td><td>{d == null ? <Pill kind="bad">never</Pill> : d >= 30 ? <Pill kind="bad">{d} days</Pill> : d >= 14 ? <Pill kind="warn">{d} days</Pill> : <Pill kind="ok">{silentText(lastSeen(p))}</Pill>}</td></tr> })}
    </tbody></table></div>
