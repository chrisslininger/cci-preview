...
await build({ entryPoints:[join(root,'tools/preview/instructors-harness.tsx')], bundle:true, format:'esm', platform:'browser', jsx:'automatic', outfile:'/tmp/harness/out/assets/h.js', define:{'process.env.NODE_ENV':'"production"'}, plugins:[alias] })
const css = readFileSync(join(root,'src/styles/tokens.css'),'utf8')+'\n'+readFileSync(join(root,'src/styles/components.css'),'utf8')
writeFileSync('/tmp/harness/out/assets/h.css', css)
const access = { person:{id:'p1',first_name:'Chris',last_name:'Slininger',credentials:'DC',email:'x@y'}, staff_role:null, tier:'member', roles:[{role_key:'executive_director'}], committees:[], can_admin_roles:true, capabilities: process.argv[2] ? process.argv[2].split(',') : ['account','member','full_admin','manage_instructors','board'] }
const fix = JSON.parse(readFileSync('/tmp/harness/fix-instructors.json','utf8'))
